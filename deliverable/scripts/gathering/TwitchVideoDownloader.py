...
if __name__ == '__main__':
    ...
    parser.add_argument('inputfile', action="store", nargs='?',
                        help='The input file, generated using TwitchVideoIDCollector. Relative to root.',
                        default='data/video-ids/top100-videos-per-user.pickle')
    parser.add_argument('--root', action="store", help='The output root folder. Default is current directory.',
                        default=os.getcwd())
    parser.add_argument('--result-file', action="store", help='The results file. Relative to root.',
    ...
    try:
        ...
        if not os.access(opts.root, os.W_OK):
            logger.critical(
                "The root direcotry {} should be writable by the process.".format(os.path.realpath(opts.root)))
            exit(1)

        if not os.access(os.path.join(opts.root, opts.inputfile), os.R_OK):
            logger.critical("Input file {} should be readable by the process.".format(
                os.path.realpath(os.path.join(opts.root, opts.inputfile))))
            exit(1)

        if not os.access(os.path.join(opts.root, opts.result_file), os.W_OK) and not os.access(
                os.path.dirname(os.path.join(opts.root, opts.result_file)), os.W_OK):
            logger.critical("Result file {} or the directory {} should be writable by the process.".format(
                os.path.realpath(os.path.join(opts.root, opts.result_file)),
                os.path.realpath(os.path.dirname(os.path.join(opts.root, opts.result_file)))))
            exit(1)
        ...
        for user in top100_videos:
            ...
            for v in top100_videos[user]['videos']:
                if v['duplicate']:
                    logger.debug(
                        'Passed duplicate video {} from {} with length {} from {}.'.format(v['id'], user, v['length'],
                                                                                           v['recorded_at']))
                    skipped += 1
                    continue
                added += 1
                pool.apply_async(ProcessVideo, args=(queue, worker_configurer, v, opts.root, opts.skip_stream),
                                 callback=log_result, error_callback=log_error)
                logger.debug('Found video {} from {} with length {} from {}.'.format(v['id'], user, v['length'],
                                                                                     v['recorded_at']))
